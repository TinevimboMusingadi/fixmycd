      ...
      if (gps) {
        try {
          ...
          if (response.ok) {
            ...
            if (data.city && data.city !== 'Unknown') setCity(data.city);
            if (data.stateProvince && data.stateProvince !== 'Unknown') setStateProvince(data.stateProvince);
            if (data.postalCode) setPostalCode(data.postalCode);
            if (
              data.county &&
              data.county !== 'Unknown' &&
              data.county !== 'Unknown County'
            ) {
              setAddressLine(prev => prev || `County: ${data.county}`);
            }
            setGpsDetected(true);
          }
        } catch (geoError) {
    ...
    try {
      const res = await fetch('/api/reports', {
        ...
        body: JSON.stringify({
          ...
          costEstimateSource:
            estimatedCostLow || estimatedCostHigh ? 'citizen_estimate' : undefined,
        }),
      });
      if (res.ok) {
        onSuccess();
      ...
      <div className="modal-content modal-content-wide">
        ...
        <div className="report-form-steps">
          {steps.map((label, i) => (
            <span
              key={label}
              className={`report-form-step ${i === step ? 'active' : ''} ${
                i < step ? 'done' : ''
              }`}
            >
              {i + 1}. {label}
          ...
          <div className="form-step">
            <div className="form-group">
              ...
              <input
                ...
                required
                placeholder="e.g. Collapsed storm drain on 14th St"
              />
            </div>
            <div className="form-group">
              ...
              <textarea
                ...
                placeholder="What failed, when, and who is affected?"
              />
            </div>
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                <select
                  value={infrastructureClass}
                  onChange={(e) => {
                    setInfrastructureClass(e.target.value);
                    setInfrastructureType('');
                  }}
                >
                  <option value="">Select class</option>
                  {Object.keys(INFRASTRUCTURE_TAXONOMY).map((c) => (
            ...
            <div className="form-group">
              ...
              <PinPickerMap
                ...
                onLocationChange={(location) => {
                  if (location.city && location.city !== 'Unknown') setCity(location.city);
                  if (location.stateProvince && location.stateProvince !== 'Unknown')
                    setStateProvince(location.stateProvince);
                  if (location.postalCode) setPostalCode(location.postalCode);
                  if (
                    location.county &&
                    location.county !== 'Unknown' &&
                    location.county !== 'Unknown County'
                  ) {
                    setAddressLine(prev => prev || `County: ${location.county}`);
                  }
                }}
              />
          ...
          <div className="form-step">
            <div className="form-row">
              <div className="form-group">
                ...
                <input
                  type="number"
                  min="0"
                  value={estimatedCostLow}
                  onChange={(e) => setEstimatedCostLow(e.target.value)}
                />
              </div>
              <div className="form-group">
                ...
                <input
                  type="number"
                  min="0"
                  value={estimatedCostHigh}
                  onChange={(e) => setEstimatedCostHigh(e.target.value)}
                />
              </div>
            </div>
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) => setOutageDurationHours(e.target.value)}
                />
              </div>
            </div>
            <div className="form-row">
            ...
            <div className="impact-flags">
              ...
              <label>
                <input
                  type="checkbox"
                  checked={accessibilityImpact}
                  onChange={(e) => setAccessibilityImpact(e.target.checked)}
                />{' '}
              ...
              <label>
                <input
                  type="checkbox"
                  checked={environmentalImpact}
                  onChange={(e) => setEnvironmentalImpact(e.target.checked)}
                />{' '}
                ...
                <div className="form-row">
                  ...
                  <div className="form-group">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setObservationConfidence(parseInt(e.target.value, 10))
                      }
                    >
                      {[1, 2, 3, 4, 5].map((n) => (
        ...
        {mode === 'advanced' && step === 4 && (
          <div className="form-step review-summary">
            ...
            <p>
              <strong>{title}</strong> · Severity {severity}
            </p>
            <p>
              {infrastructureClass}
              {infrastructureType ? ` → ${infrastructureType}` : ''} ·{' '}
              {failureType || 'Unspecified failure'}
            </p>
            <p>
              {city || 'City TBD'}
              {stateProvince ? `, ${stateProvince}` : ''} · {latitude.toFixed(4)},{' '}
              {longitude.toFixed(4)}
            </p>
            {(estimatedCostLow || estimatedCostHigh) && (
              <p>
                Est. cost: ${estimatedCostLow || '?'} – ${estimatedCostHigh || '?'}
              </p>
            )}
